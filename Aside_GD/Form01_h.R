...
server <- function(input, output, session) {
  ...
  # Lógica para mostrar/ocultar el campo de "Otra Moneda"
  observeEvent(input$moneda_ultima_orden, {
    if (input$moneda_ultima_orden == "Otro") {
      shinyjs::show("moneda_ultima_orden_otro")
    } else {
      shinyjs::hide("moneda_ultima_orden_otro")
      updateTextInput(session, "moneda_ultima_orden_otro", value = "")
    }
  })
  ...
  clear_fields <- function() {
    ...
    updateNumericInput(session, "mgta", value = 0)
    updateNumericInput(session, "monto_ultima_orden", value = 0)
  ...
  observeEvent(input$modify_button, {
    ...
    updateNumericInput(session, "mgta", value = fila_datos$MGTA)
    updateNumericInput(session, "monto_ultima_orden", value = fila_datos$Monto_ultima_orden)
    updateRadioButtons(session, "moneda_ultima_orden", selected = fila_datos$Moneda_ultima_orden)
    updateTextInput(session, "moneda_ultima_orden_otro", value = fila_datos$Moneda_ultima_orden_Otro)
    # ... (resto de updates)
