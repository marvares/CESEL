...
ui <- fluidPage(
  ...
  checkboxGroupInput("r2c", "Reason to call (R2C):",
                     ...
                     inline = TRUE),
  shinyjs::hidden(textInput("r2c_otro", "Especificar 'Other':", "")),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$submit, {
    # Para asegurar que todos los campos existan al crear el dataframe
    new_data <- data.frame(
  ...
  observeEvent(input$cancel, {
    clear_fields()
    output$form_title <- renderText("Agregar Nuevo Cliente")
    shinyjs::show("submit"); shinyjs::hide("update"); shinyjs::show("cancel")
  })
